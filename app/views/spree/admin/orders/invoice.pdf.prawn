...
grid([1,0], [6,4]).bounding_box do
    ...
    @order.payments.each do |payment|
        ...
        totals << [make_cell( :content => t(:order_payment_via, :gateway => p_gateway, :number => p_id, :date => p_date), :font_style => :bold), number_to_currency(payment.amount)]
        total_payments += payment.amount
    end
    totals << [make_cell( :content => t(:balance), :font_style => :bold), number_to_currency(@order.total - total_payments)]
    ...
    move_down 4
    text I18n.t(:return_message), :align => :left, :size => 9
...
repeat(:all) do    
    grid([7,0], [7,4]).bounding_box do
        ...
        footer1 =  [
            make_cell( :content => t(:footer_left1) ),
            make_cell( :content => t(:footer_right1) ),
        ]
        footer2 =  [
